...
class CrossdataSimulation extends Simulation with Common {
  ...
  val list = Seq[ActionBuilder]

  val scn = scenario("Crossdata perfomance protocol test").exec(mine)
...
class CrossdataAction(val next: Action, ctx: ScenarioContext, user: String, password: String ,query: String) extends ChainableAction with Common {
...
/*    try {*/
      driver = Driver.http.newSession(user, password, driverConf)
